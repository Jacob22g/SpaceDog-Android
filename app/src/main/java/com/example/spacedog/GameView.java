...
//        highScore[3] = sharedPreferences.getInt("score4",0);
        ...
        for(int i=0; i<enemyCount; i++){
            enemies[i] = new Enemy(context, screenX, screenY, level);
        }
...
//            friends[i] = new Friend(context, screenX, screenY);
            friends[i] = new Friend(context, screenX, screenY, level);
        }


        // initializing the media players for the game sounds
        // set music for the specific level
        if (level == 1)
            gameBackgroundMusic = MediaPlayer.create(context,R.raw.lvl1music);
        else if (level == 2)
            gameBackgroundMusic = MediaPlayer.create(context,R.raw.lvl2music);
        else
            gameBackgroundMusic = MediaPlayer.create(context,R.raw.lvl3music);
...
//                canvas.drawColor(context.getResources().getColor(R.color.background));
            ...
            if (level == 1)
                //drawing a background color for canvas
                if (Build.VERSION.SDK_INT >= 23)
                    canvas.drawColor(getResources().getColor(R.color.level_1_background , null));
                else
                    canvas.drawColor(context.getResources().getColor(R.color.level_1_background));
            else if (level == 2)
                //drawing a background color for canvas
                if (Build.VERSION.SDK_INT >= 23)
                    canvas.drawColor(getResources().getColor(R.color.level_2_background , null));
                else
                    canvas.drawColor(context.getResources().getColor(R.color.level_2_background));
            else
                //drawing a background color for canvas
                if (Build.VERSION.SDK_INT >= 23)
                    canvas.drawColor(getResources().getColor(R.color.level_3_background , null));
                else
                    canvas.drawColor(context.getResources().getColor(R.color.level_3_background));
